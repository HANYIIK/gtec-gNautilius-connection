...
def Initialize(
    ...
    def strip_gds(d):
        return {k.replace('GDS_', ''): v for k, v in d.items()}
...
class _ffi_struct_wrap(object):
    ...
    def _to_python(self):
        ...
        return {k: __to_python(v) for k, v in self.__dict__.items() if
                not k.startswith('_') and k not in self._name_map}
...
class GDS(_config_wrap):
    ...
    *server_ip*: the IP address of the GDS server. Default: pygds.SERVER_IP
    ...
    def _getdata(self):
        ...
        try:
            ...
            while True:
                ...
                # print("in2")
                start = time.time()
                while self.do and scantotal < scanCount:  # 250 （样本点）
                    sc[0] = 0
                    ...
                    # end = time.time()
                    # print("acuqire time: %0.3f" %(end-start))
                    if sc[0] > 0:
                        scantotal += sc[0]
                        loops = 0
                    else:
        ...
        # sc:A pointer to a variable that either holds the number of scans to read.
        # If variable pointed to holds zero, the method reads as many scans as either currently available or the specified destination buffer can contain, whichever is smaller.
        # This variable receives the number of actually read scans when the method returns.
        # fa:The buffer where the read scans are written to.
        # left:The maximum number of elements that \a destinationBuffer can contain. See \ref GDS_GetDataInfo for details on the definition of buffer elements.
    ...
    def GetAvailableChannels(self,
                             combine=True
                             # and-combine the C API's GDS_XXX_GetAvailableChannels with the channel's Acquire flag
                             ):
        '''GDS.
